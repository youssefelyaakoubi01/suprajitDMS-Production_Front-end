...
interface ProductionLineHistory {
    id: number;
...
@Component({
    selector: 'app-production-lines-history',
    standalone: true,
    imports: [
        CommonModule,
        FormsModule,
        ...
        <div class="card">
            <p-toolbar styleClass="mb-4">
                <ng-template pTemplate="left">
                    <h2 class="m-0">
                        <i class="pi pi-history mr-2"></i>Production Lines History
                    </h2>
                </ng-template>
                ...
                *ngIf="!timelineView"
                ...
                <ng-template pTemplate="header">
                    <tr>
                        ...
                        <th pSortableColumn="production_line_name">
                            Production Line <p-sortIcon field="production_line_name"></p-sortIcon>
                        </th>
                ...
                <ng-template pTemplate="body" let-record>
                    <tr>
                        ...
                        <td>
                            <strong class="text-primary">{{ record.production_line_name }}</strong>
                        </td>
                        <td>
                            <p-tag
                        ...
                        <p-card [styleClass]="'timeline-card ' + event.change_type + '-card'">
                            <ng-template pTemplate="header">
                                <div class="timeline-card-header">
                                    ...
                                    <strong class="text-primary ml-2">{{ event.production_line_name }}</strong>
                                </div>
                            </ng-template>
...
export class ProductionLinesHistoryComponent implements OnInit {
    history: ProductionLineHistory[] = [];
    filteredHistory: ProductionLineHistory[] = [];
    groupedHistory: ProductionLineHistory[] = [];
    users: string[] = [];
    ...
    loadData(): void {
        ...
        forkJoin({
            history: this.productionService.getProductionLineHistory(),
            users: this.productionService.getProductionLineHistoryUsers()
        }).subscribe({
            next: (data: any) => {
    ...
    groupHistoryForTimeline(history: ProductionLineHistory[]): ProductionLineHistory[] {
    ...
    exportToCSV(): void {
        ...
        // Prepare CSV content
        const headers = ['Date/Time', 'Production Line', 'Action', 'Field', 'Old Value', 'New Value', 'User'];
        ...
        link.setAttribute('download', `production_lines_history_${new Date().toISOString().slice(0, 10)}.csv`);
