        ...
        <div class="card">
            <p-toolbar styleClass="mb-4">
                <ng-template pTemplate="left">
                    <h2 class="m-0">
                        <i class="pi pi-history mr-2"></i>Parts History
                    </h2>
                </ng-template>
                        ...
                        <p-card [styleClass]="'timeline-card ' + event.change_type + '-card'">
                            <ng-template pTemplate="header">
                                <div class="timeline-card-header">
                                    ...
                                    <strong class="text-primary ml-2">{{ event.part_number }}</strong>
                                </div>
                            </ng-template>
...
export class PartsHistoryComponent implements OnInit {
    history: PartHistory[] = [];
    filteredHistory: PartHistory[] = [];
    groupedHistory: PartHistory[] = [];
    projects: Project[] = [];
    users: string[] = [];
    ...
    loadData(): void {
        ...
        forkJoin({
            history: this.productionService.getPartHistory(),
            projects: this.productionService.getProjects(),
            users: this.productionService.getPartHistoryUsers()
        }).subscribe({
            next: (data: any) => {
    ...
    applyFilters(): void {
        ...
        if (this.selectedProject) {
            const projectName = this.projects.find(p => p.id === this.selectedProject)?.name;
            if (projectName) {
                result = result.filter(h => h.project_name === projectName);
            }
        }
    ...
    groupHistoryForTimeline(history: PartHistory[]): PartHistory[] {
        // For timeline view, we may want to group consecutive changes to the same part
        // For now, just return the filtered history sorted by date desc
    ...
    exportToCSV(): void {
        ...
        // Prepare CSV content
        const headers = ['Date/Time', 'Part Number', 'Project', 'Action', 'Field', 'Old Value', 'New Value', 'User'];
        ...
        link.setAttribute('download', `parts_history_${new Date().toISOString().slice(0, 10)}.csv`);
