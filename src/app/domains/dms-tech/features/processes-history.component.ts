        ...
        <div class="card">
            <p-toolbar styleClass="mb-4">
                <ng-template pTemplate="left">
                    <h2 class="m-0">
                        <i class="pi pi-history mr-2"></i>Processes History
                    </h2>
                </ng-template>
                ...
                *ngIf="!timelineView"
                ...
                <ng-template pTemplate="body" let-record>
                    <tr>
                        ...
                        <td>
                            <strong class="text-primary">{{ record.process_name }}</strong>
                        </td>
                        <td>
                            <p-tag
                        ...
                        <p-card [styleClass]="'timeline-card ' + event.change_type + '-card'">
                            <ng-template pTemplate="header">
                                <div class="timeline-card-header">
                                    ...
                                    <strong class="text-primary ml-2">{{ event.process_name }}</strong>
                                </div>
                            </ng-template>
...
export class ProcessesHistoryComponent implements OnInit {
    ...
    filteredHistory: ProcessHistory[] = [];
    groupedHistory: ProcessHistory[] = [];
    users: string[] = [];
    ...
    loadData(): void {
        ...
        forkJoin({
            history: this.productionService.getProcessHistory(),
            users: this.productionService.getProcessHistoryUsers()
        }).subscribe({
            next: (data: any) => {
    ...
    groupHistoryForTimeline(history: ProcessHistory[]): ProcessHistory[] {
    ...
    exportToCSV(): void {
        ...
        // Prepare CSV content
        const headers = ['Date/Time', 'Process Name', 'Action', 'Field', 'Old Value', 'New Value', 'User'];
        ...
        link.setAttribute('download', `processes_history_${new Date().toISOString().slice(0, 10)}.csv`);
